...
3. [DONE 2026-08-15] Get real analytics wired up (GA4 + Search Console) — without it, revenue/traffic projections stay guesses.
   GA4: initial implementation loaded gtag.js directly with a hand-rolled dataLayer stub (measurement ID G-TCQTDGKSY5) — script loaded fine, CSP allowed it (after fixing a real production bug where CSP was silently blocking googletagmanager.com/google-analytics.com entirely, in both web/nginx.conf and aadikarta_nginx.conf), but Tag Assistant showed zero hits ever sent. Root cause: this Google account's Tag Manager expects the site installed via a full GTM container (GTM-WGS7TLDK — confirmed via Tag Manager's own "Installation instructions"), not a raw gtag.js include. Switched to the GTM snippet Google specified, in web/index.html (head script + body noscript iframe), added frame-src googletagmanager.com to CSP for the noscript iframe, and simplified analytics.ts/Analytics.tsx to just push page_view events into GTM's own dataLayer on SPA route changes (GTM boots itself now; VITE_GA_MEASUREMENT_ID env var removed as unused — GA4 config lives inside the GTM container on Google's side, not our code). The GTM container itself was empty (no tags at all) — walked through creating the Google Tag (G-TCQTDGKSY5) with two firing triggers (Initialization - All Pages, and a new Custom Event trigger matching our page_view push) in Tag Manager's UI and publishing it (Version 3, live 2026-08-15). VERIFIED WORKING: GA4 Realtime showed a real active user with correct page views ("Free Astrology & Vedic Insights", 9 views) and geolocation after publishing — confirmed end-to-end, hits are genuinely flowing now. The historical GA4 data from before Aug 15 remains unexplained (unrelated stray data) — don't trust it, but current tracking is confirmed live and correct.
   Search Console: turns out already done before this session (verified property + sitemap.xml submitted Aug 14, 2026, status Success, 76 pages discovered) — found via the GSC UI, not something built this session. VITE_GSC_VERIFICATION in web/.env.production is still blank/unused since verification happened some other way (DNS or prior HTML-tag setup outside this codebase) — harmless to leave as-is.
